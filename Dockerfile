...
FROM ubuntu:latest
# Set version label
LABEL maintainer="github.com/aperep"
LABEL image="MTProxy"
...
# Install dependencies, you would need common set of tools.
    apt -y install git curl build-essential libssl-dev zlib1g-dev > /dev/null 2>&1;\
    #  cron wget logrotate ntp
...
RUN curl -s https://core.telegram.org/getProxySecret -o /srv/MTProxy/objs/bin/proxy-secret &&\
...
RUN useradd -ms /bin/bash mtproxy
